...
// .wrangler/tmp/bundle-1kBddy/strip-cf-connecting-ip-header.js
...
var IN_MEMORY_MAX_SIZE = 500;
function memSet(key, raw) {
...
var Cache = class {
   ...
   */
  async write(key, value, ttlFreshSeconds, ttlStaleSeconds = 3600) {
    const now = /* @__PURE__ */ new Date();
    const freshUntil = new Date(now.getTime() + ttlFreshSeconds * 1e3);
    const staleUntil = new Date(now.getTime() + (ttlFreshSeconds + ttlStaleSeconds) * 1e3);
    const entry = {
      value,
...
// src/marketHours.ts
function inferMarketFromTicker(declaredType, ticker) {
...
__name(inferMarketFromTicker, "inferMarketFromTicker");
function toZoned(now, timeZone) {
...
__name(toZoned, "toZoned");
function isWithin(hour, minute, startH, startM, endH, endM) {
...
function isMarketOpen(category, now = /* @__PURE__ */ new Date()) {
  switch (category) {
    case "crypto":
      return true;
    case "us_stock":
    ...
    case "cn_stock": {
      ...
      if (weekday === 0 || weekday === 6)
        return false;
      return isWithin(hour, minute, 9, 30, 11, 30) || isWithin(hour, minute, 13, 0, 15, 0);
    }
    case "hk_stock": {
      const { weekday, hour, minute } = toZoned(now, "Asia/Hong_Kong");
      if (weekday === 0 || weekday === 6)
        return false;
      return isWithin(hour, minute, 9, 30, 12, 0) || isWithin(hour, minute, 13, 0, 16, 0);
    }
    case "forex":
    case "cash": {
...
function getCacheTtlSeconds(category, now = /* @__PURE__ */ new Date()) {
  return isMarketOpen(category, now) ? 30 : 600;
}
__name(getCacheTtlSeconds, "getCacheTtlSeconds");

// src/providers/http.ts
var KERDOS_UA = "Mozilla/5.0 (compatible; KerdosQuoteRelay/1.0; +https://kerdos.app)";
function kerdosFetch(url, init) {
  const { timeoutMs = 8e3, headers, ...rest } = init ?? {};
...
__name(kerdosFetch, "kerdosFetch");
...
function tickerToCoinId(ticker) {
  return TICKER_TO_ID[ticker.toUpperCase()] ?? null;
}
__name(tickerToCoinId, "tickerToCoinId");
async function fetchCoinGeckoPrice(ticker) {
  ...
  const res = await kerdosFetch(url, { timeoutMs: 8e3 });
  if (!res.ok) {
  ...
  const asOf = priceBlock.last_updated_at ? new Date(priceBlock.last_updated_at * 1e3).toISOString() : (/* @__PURE__ */ new Date()).toISOString();
  return {
...
__name(searchCoinGecko, "searchCoinGecko");
async function fetchCoinGeckoBenchmark(id, days) {
  const url = `${BASE}/coins/${encodeURIComponent(id)}/market_chart?vs_currency=usd&days=${days}`;
  const res = await kerdosFetch(url, { timeoutMs: 1e4 });
  if (!res.ok) {
...
__name(fetchCoinGeckoBenchmark, "fetchCoinGeckoBenchmark");

// src/providers/forex.ts
async function fetchExchangeRateApi(base) {
  ...
  const asOf = data.time_last_update_unix ? new Date(data.time_last_update_unix * 1e3).toISOString() : (/* @__PURE__ */ new Date()).toISOString();
  return {
    ticker: b,
    price: usdRate,
    currency: "USD",
...
__name(fetchExchangeRateApi, "fetchExchangeRateApi");
async function fetchFrankfurter(base) {
  ...
  return {
    ...
    currency: "USD",
    source: "frankfurter",
    asOf: (/* @__PURE__ */ new Date(`${data.date}T00:00:00Z`)).toISOString()
  };
}
__name(fetchFrankfurter, "fetchFrankfurter");
async function fetchForex(base) {
  try {
...
__name(fetchForex, "fetchForex");

// src/providers/stooq.ts
var QUOTE_BASE = "https://stooq.com/q/l/";
var HIST_BASE = "https://stooq.com/q/d/l/";
function yahooToStooqSymbol(yahooTicker) {
  const t = yahooTicker.toLowerCase();
  if (t.endsWith(".ss"))
    return t.replace(".ss", ".sh");
  if (t.endsWith(".sz") || t.endsWith(".hk"))
    return t;
  if (t.startsWith("^")) {
    const m = {
      "^gspc": "^spx",
...
__name(yahooToStooqSymbol, "yahooToStooqSymbol");
...
__name(parseQuoteCsv, "parseQuoteCsv");
async function fetchStooqPrice(yahooTicker) {
  ...
  let currency = "USD";
  if (stooqSym.endsWith(".sh") || stooqSym.endsWith(".sz"))
    currency = "CNY";
  else if (stooqSym.endsWith(".hk"))
    currency = "HKD";
  return {
...
__name(fetchStooqPrice, "fetchStooqPrice");
async function fetchStooqHistorical(yahooTicker, days) {
  const stooqSym = yahooToStooqSymbol(yahooTicker);
  const now = /* @__PURE__ */ new Date();
  const start = new Date(now.getTime() - days * 24 * 3600 * 1e3);
  ...
  const res = await kerdosFetch(url, { timeoutMs: 15e3 });
  if (!res.ok) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!Number.isFinite(close) || close === 0)
      continue;
    points.push({ t: (/* @__PURE__ */ new Date(`${cols[0]}T00:00:00Z`)).toISOString(), v: close });
  }
  if (points.length === 0) {
...
__name(fetchStooqHistorical, "fetchStooqHistorical");

// src/providers/yahoo.ts
var CHART_BASE = "https://query1.finance.yahoo.com/v8/finance/chart";
var SEARCH_BASE = "https://query2.finance.yahoo.com/v1/finance/search";
...
__name(normalizeYahooTicker, "normalizeYahooTicker");
async function fetchYahooPrice(ticker) {
  ...
  const asOf = result.meta.regularMarketTime ? new Date(result.meta.regularMarketTime * 1e3).toISOString() : (/* @__PURE__ */ new Date()).toISOString();
  return {
    ticker: normalized,
    price,
...
__name(fetchYahooPrice, "fetchYahooPrice");
async function fetchYahooHistorical(symbol, days) {
  const range = days <= 32 ? "1mo" : days <= 95 ? "3mo" : days <= 190 ? "6mo" : days <= 370 ? "1y" : days <= 740 ? "2y" : "5y";
  ...
  const points = timestamps.map((t, i) => ({
    t: new Date(t * 1e3).toISOString().slice(0, 10),
    v: closes[i]
...
__name(fetchYahooHistorical, "fetchYahooHistorical");
...
var memoryRateMap = /* @__PURE__ */ new Map();
async function rateLimit(env2, ip) {
  const limit = Number(env2.RATE_LIMIT_PER_MIN ?? "60");
  const windowSec = 60;
  const now = Date.now();
  const windowKey = Math.floor(now / (windowSec * 1e3));
  const key = `rl:${ip}:${windowKey}`;
  const resetAt = (windowKey + 1) * windowSec * 1e3;
  if (env2.RATE_LIMIT) {
    try {
      ...
      await env2.RATE_LIMIT.put(key, String(count3 + 1), { expirationTtl: windowSec * 2 });
      return { allowed: true, remaining: Math.max(0, limit - count3 - 1), resetAt };
    } catch (err) {
...
function getClientIp(req) {
  return req.headers.get("cf-connecting-ip") || req.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || req.headers.get("x-real-ip") || "anonymous";
}
__name(getClientIp, "getClientIp");
var priceQuerySchema = external_exports.object({
  ...
  ticker: external_exports.string().min(1).max(32)
});
async function fetchPriceFromProvider(type, ticker) {
  switch (type) {
    case "us_stock":
    case "cn_stock":
    case "hk_stock":
...
__name(fetchPriceFromProvider, "fetchPriceFromProvider");
async function handlePrice(req, env2) {
  ...
  const cache = new Cache(env2.PRICE_CACHE);
  ...
  const cacheRead = await cache.read(cacheKey);
  ...
  try {
    ...
    return jsonResponse(toPriceResponse(fresh, (/* @__PURE__ */ new Date()).toISOString(), false));
  } catch (err) {
...
function toPriceResponse(p, cachedAt, stale) {
  return {
    ticker: p.ticker,
    price: p.price,
    currency: p.currency,
...
__name(toPriceResponse, "toPriceResponse");
var searchQuerySchema = external_exports.object({
  type: external_exports.enum(["stock", "crypto", "auto"]),
  q: external_exports.string().min(1).max(64)
});
async function searchAuto(q) {
...
function score(entry, qLower) {
  const t = entry.ticker.toLowerCase();
  const n = entry.name.toLowerCase();
  if (t === qLower)
    return 100;
  if (t.startsWith(qLower))
    return 80;
...
var benchmarkQuerySchema = external_exports.object({
  id: external_exports.enum(["sp500", "csi300", "btc"]),
  range: external_exports.enum(["1m", "3m", "1y", "all"])
});
...
__name(rangeToDays, "rangeToDays");
async function fetchBenchmark(id, days) {
  switch (id) {
    case "sp500":
      try {
...
__name(fetchBenchmark, "fetchBenchmark");
async function handleBenchmark(req, env2) {
  const url = new URL(req.url);
  ...
  try {
    ...
    return jsonResponse({ ...series, cachedAt: (/* @__PURE__ */ new Date()).toISOString(), stale: false });
  } catch (err) {
  ...
  async fetch(req, env2, _ctx) {
    ...
    if (path === "/healthz") {
      return jsonResponse({
        ...
        kv: {
          PRICE_CACHE: Boolean(env2.PRICE_CACHE),
          RATE_LIMIT: Boolean(env2.RATE_LIMIT)
        }
      });
    }
    const ip = getClientIp(req);
    const rl = await rateLimit(env2, ip);
    if (!rl.allowed) {
      ...
      return jsonResponse(
        ...
        429,
        {
          ...
          "X-RateLimit-Reset": String(Math.floor(rl.resetAt / 1e3))
        }
      );
    }
    try {
      switch (path) {
        case "/price":
          return await handlePrice(req, env2);
        case "/search":
          return await handleSearch(req, env2);
        case "/benchmark":
          return await handleBenchmark(req, env2);
        default:
...
// .wrangler/tmp/bundle-1kBddy/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-1kBddy/middleware-loader.entry.ts
